...
// Query to get sales by employee
$sql = "SELECT h.emp_id, a.aname as emp_name, SUM(h.pro_value) as total_sales
        FROM ot_head h
...
while ($row = $res->fetch_assoc()) {
    ...
    $commission_val = ($total_sales * $emp_commission_pct) / 100;
    
    $row['commission_pct'] = $emp_commission_pct;
...
<div class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark"><i class="fas fa-user-tie text-info ml-2"></i> تحقيق مبيعات الموظفين</h1>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="container-fluid">
            ...
            <div class="alert alert-secondary border-0 shadow-sm d-flex align-items-center justify-content-between p-3 mb-4" style="background-color: #f8f9fa;">
                <div>
                    ...
                    <strong>نسبة عمولة الموظفين الافتراضية:</strong> 
                    <span class="badge badge-info px-3 py-2" style="font-size: 1rem;"><?= number_format($emp_commission_pct, 2) ?> %</span>
                </div>
                                ...
                                <?php if (count($data) > 0): ?>
                                    <?php $x = 0; foreach ($data as $row): $x++; ?>
                                        <tr class="text-center">
                                            <td><?= $x ?></td>
                                            <td class="text-right font-weight-bold"><?= htmlspecialchars($row['emp_name']) ?></td>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="5" class="text-center text-muted p-4">
                                            ...
                                            لا توجد بيانات مبيعات للموظفين خلال هذه الفترة.
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
